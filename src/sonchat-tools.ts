...
export const sonchatTools: Tool[] = [
  {
    name: "sonchat_health",
  ...
  },
  {
    ...
    description:
      "List all active invite tokens for SonChat. Requires SONCHAT_ADMIN_KEY.",
    inputSchema: { type: "object", properties: {} },
  },
  {
  ...
  },
  {
    ...
    description:
      "Revoke/delete a SonChat invite token. Requires SONCHAT_ADMIN_KEY.",
    inputSchema: {
      type: "object",
      properties: {
        token: {
...
function noAdminKey(): TextResult {
  return ok({
...
export async function handleSonchatTool(
  name: string,
  ...
  switch (name) {
    ...
    case "sonchat_activity": {
      if (!SONCHAT_ADMIN_KEY) return noAdminKey();
      const { limit = 100 } = args as { limit?: number };
      try {
        const res = await sonchatFetch(`/admin/activity?limit=${limit}`, {
          headers: adminHeaders(),
        });
        if (!res.ok) return ok({ error: `SonChat returned HTTP ${res.status}` });
    ...
    case "sonchat_create_invite": {
      ...
      const {
        ...
        expires_in_days = 7,
        max_uses,
      } = args as { label?: string; expires_in_days?: number; max_uses?: number };

      const body: Record<string, unknown> = { label, expiresInDays: expires_in_days };
      if (max_uses != null) body.maxUses = max_uses;
    ...
    case "sonchat_revoke_invite": {
      ...
      try {
        const res = await sonchatFetch(
          `/admin/invite/${encodeURIComponent(token)}`,
          { method: "DELETE", headers: adminHeaders() }
        );
        if (!res.ok) return ok({ error: `SonChat returned HTTP ${res.status}` });
        let result: unknown = { success: true };
        try { result = await res.json(); } catch { /* may return no body */ }
        return ok(result);
      } catch (err) {
        return ok({
          error: `Cannot reach SonChat at ${SONCHAT_BASE}`,
