...
export const memoryTools: Tool[] = [
  {
    ...
    description:
      "Get user facts from AION's long-term memory. " +
  ...
  },
  {
    name: "memory_search",
    description:
      "Search AION's memory for a keyword. Searches across both user facts and conversation summaries. " +
  ...
  },
  {
    ...
    description:
      "Get the full memory context that AION injects into its system prompt — the last 10 conversation summaries plus all user facts, " +
  ...
  },
  {
    ...
    description:
      "Get memories about Jenn from AION's Jenn memory database. " +
...
export async function handleMemoryTool(
  ...
  switch (name) {
    ...
    case "memory_get_facts": {
      const { category, limit = 200 } = args as { category?: string; limit?: number };
    ...
    case "memory_add_fact": {
      const { category, fact } = args as { category: string; fact: string };
      try {
        const res = db
    ...
    case "memory_search": {
      ...
      const convs = db
        .prepare(
          "SELECT id, title, tool, summary, created_at FROM conversations " +
      ...
      return ok({
        query,
        facts_found: (facts as unknown[]).length,
        facts,
        conversations_found: (convs as unknown[]).length,
        conversations: convs,
      });
    }
    ...
    case "memory_get_context": {
      const summaries = db
        .prepare(
          "SELECT summary FROM conversations WHERE summary IS NOT NULL ORDER BY updated_at DESC LIMIT 10"
        )
        .all() as { summary: string }[];

      const facts = db
        .prepare("SELECT category, fact FROM user_facts ORDER BY category, created_at")
        .all() as { category: string; fact: string }[];
    ...
    case "conversation_list": {
      const { limit = 20, tool } = args as { limit?: number; tool?: string };
      const params: unknown[] = [];
      let sql =
        "SELECT id, title, tool, mode, created_at, updated_at, summary FROM conversations";
      if (tool) {
        sql += " WHERE tool = ?";
        params.push(tool);
      }
      ...
      return ok({ total: (rows as unknown[]).length, conversations: rows });
    }
    ...
    case "conversation_get": {
      ...
      return ok({ conversation: conv, message_count: (messages as unknown[]).length, messages });
    }
    ...
    case "conversation_search": {
      ...
      return ok({ query, results: (rows as unknown[]).length, conversations: rows });
    }
    ...
    case "jenn_get_memories": {
      ...
      return ok({ total: (rows as unknown[]).length, memories: rows });
    }
