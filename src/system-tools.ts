...
export const systemTools: Tool[] = [
  {
    name: "system_status",
    description:
      "Check the health of all Drayhub services: Ollama (LLM backend), Aion Flask API (port 5000), " +
...
async function probe(url: string, label: string, timeoutMs = 4000): Promise<ServiceResult> {
    ...
    case "system_ollama_models": {
      try {
        ...
        const data = await res.json() as {
          models?: { name: string; size: number; modified_at: string; details?: unknown }[];
        };
