...
// ---------------------------------------------------------------------------
async function execInKali(cmd: string, timeoutMs = DEFAULT_TIMEOUT_MS): Promise<string> {
  if (!KALI_PASS) {
...
export const kaliTools: Tool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        wordlist: {
          type: "string",
          description:
            "Path to wordlist inside Kali container (default: /usr/share/wordlists/dirb/common.txt)",
        },
      },
      required: ["url"],
    },
  ...
  },
  {
    name: "kali_status",
    description:
      "Check that the Kali Docker container can be reached and return the Kali version / tool inventory.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
...
// Tool handler
// ---------------------------------------------------------------------------
export async function handleKaliTool(
  name: string,
    ...
    switch (name) {
      ...
      case "kali_dir_enum": {
        ...
        const out = await execInKali(
          `gobuster dir -u ${url} -w ${wl} -q --no-progress -t 20`
        );
      ...
      case "kali_status": {
        const out = await execInKali(
          `echo "=== Kali Version ===" && cat /etc/os-release | grep -E "^(NAME|VERSION)" && echo "=== Key Tools ===" && for t in nmap nikto gobuster sqlmap hydra metasploit-framework; do echo -n "$t: "; which $t 2>/dev/null && $t --version 2>&1 | head -1 || echo "not found"; done`
        );
