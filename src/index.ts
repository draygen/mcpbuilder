...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (memoryToolNames.has(name)) return await handleMemoryTool(name, a);
    if (systemToolNames.has(name)) return await handleSystemTool(name, a);
    if (chatToolNames.has(name)) return await handleChatTool(name, a);
    if (kaliToolNames.has(name)) return await handleKaliTool(name, a);
    if (aionApiToolNames.has(name)) return await handleAionApiTool(name, a);
    if (sonchatToolNames.has(name)) return await handleSonchatTool(name, a);
    if (portalToolNames.has(name)) return await handlePortalTool(name, a);
    if (fleetToolNames.has(name)) return await handleFleetTool(name, a);
