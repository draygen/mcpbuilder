...
export const chatTools = [
    {
        name: "aion_query",
        description: "Send a message to AION via its public REST API (port 3003). " +
    ...
    {
        name: "ollama_query",
        description: "Send a raw prompt directly to Ollama, bypassing AION's persona and memory. " +
    ...
    {
        name: "openai_query",
        description: "Send a raw prompt directly to OpenAI (GPT-4o by default), bypassing AION's persona and memory. " +
    ...
    {
        name: "image_generate",
        description: "Generate an image using Fooocus (Stable Diffusion XL) running locally. " +
...
// Handler
// ---------------------------------------------------------------------------
export async function handleChatTool(name, args) {
    switch (name) {
        // -----------------------------------------------------------------------
        case "aion_query": {
            const { message } = args;
        ...
        // -----------------------------------------------------------------------
        case "ollama_query": {
            const { prompt, model = AION_MODEL, system, temperature = 0.7, } = args;
            try {
                const controller = new AbortController();
        ...
        // -----------------------------------------------------------------------
        case "openai_query": {
            const { prompt, model = OPENAI_MODEL, system, temperature = 0.7, } = args;
            if (!OPENAI_API_KEY) {
        ...
        // -----------------------------------------------------------------------
        case "image_generate": {
            const { prompt, negative_prompt = "", style = "Fooocus V2", ratio = "1152×896", performance = "Speed", } = args;
            ...
            try {
                ...
                for (let attempt = 0; attempt < maxAttempts; attempt++) {
                    await sleep(2000);
                    try {
                        const pollRes = await fetch(`${FOOOCUS_URL}/v1/generation/query-job?job_id=${jobId}`);
                        if (!pollRes.ok)
                            continue;
