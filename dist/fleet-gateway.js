...
import { runFleetStatus, handleFleetTool } from "./fleet-tools.js";
...
function authorizedWrite(req) {
    if (!WRITE_ENABLED)
        return false;
    if (!TOKEN)
        return true; // localhost-only + no token configured
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && path === "/fleet/status") {
        const stale = isStale();
        if (stale)
            void refresh(); // never block on the 60s SSH fan-out
    ...
    if (req.method === "POST" && (path === "/fleet/run" || path === "/fleet/review")) {
        if (!authorizedWrite(req)) {
            send(res, WRITE_ENABLED ? 401 : 403, { error: WRITE_ENABLED ? "invalid or missing x-fleet-token" : "writes disabled (FLEET_GATEWAY_WRITE=off)" });
            return;
        }
        let body;
        try {
...
server.listen(PORT, HOST, () => {
    console.error(`[fleet-gateway] http://${HOST}:${PORT}  (writes ${WRITE_ENABLED ? (TOKEN ? "on, token-required" : "on, localhost") : "off"})`);
    void refresh();
});
