...
// Tool definitions
// ---------------------------------------------------------------------------
export const aionApiTools = [
    {
        name: "aion_api_chat",
        description: "Send a message to Aion via the internal service chat endpoint (/api/service/chat). " +
    ...
    {
        name: "aion_channels",
        description: "List all Aion chat channels. Requires AION_SESSION_TOKEN to be set.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "aion_channel_history",
        description: "Fetch message history for a specific Aion channel. Requires AION_SESSION_TOKEN.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Get active users in a specific Aion channel. Requires AION_SESSION_TOKEN.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "aion_activity",
        description: "Get recent audit/activity events from Aion (joins, messages, logins, etc.). " +
    ...
    },
    {
        ...
        description: "Browse Aion's fact memory, optionally filtered by category. " +
            "Requires AION_SESSION_TOKEN. " +
    ...
    },
    {
        ...
        description: "List all users registered in Aion. Requires an admin session (AION_SESSION_TOKEN).",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "aion_admin_profile_rebuild",
        description: "Trigger a rebuild of Aion's cached system prompt profile (re-summarises facts with GPT-4o). " +
            "Requires an admin session (AION_SESSION_TOKEN).",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "aion_vast_instances",
        description: "List current Vast.ai GPU instances managed by Aion. " +
            "Requires AION_SESSION_TOKEN with vast/admin access.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "aion_vast_offers",
        description: "Browse available Vast.ai GPU offers. Requires AION_SESSION_TOKEN with vast/admin access.",
        inputSchema: {
            type: "object",
            properties: {
                max_price: {
    ...
    },
    {
        ...
        description: "Deploy a new Vast.ai GPU instance via Aion. Requires AION_SESSION_TOKEN with vast/admin access.",
        inputSchema: {
            type: "object",
            properties: {
                offer_id: {
    ...
    {
        name: "aion_vast_stop",
        description: "Stop a running Vast.ai GPU instance. Requires AION_SESSION_TOKEN with vast/admin access.",
        inputSchema: {
            type: "object",
            properties: {
                instance_id: {
    ...
    },
    {
        ...
        description: "Run a network intelligence command (nmap, nikto, etc.) via Aion's admin panel. " +
...
async function aionGet(path, timeoutMs = 8000) {
...
async function aionPost(path, body, extraHeaders = {}, timeoutMs = 20000) {
    const ctrl = new AbortController();
    setTimeout(() => ctrl.abort(), timeoutMs);
    return fetch(`${AION_BASE}${path}`, {
        method: "POST",
...
function noSession() {
    return ok({
        ...
        hint: "Log in to Aion at http://127.0.0.1:5000, open DevTools → Application → Cookies, " +
...
// Handler
// ---------------------------------------------------------------------------
export async function handleAionApiTool(name, args) {
    switch (name) {
        // -----------------------------------------------------------------------
        case "aion_api_chat": {
            ...
            const chatBody = { message, tts: false };
            if (channel)
                chatBody.channel = channel;
            try {
                const res = await aionPost("/api/service/chat", chatBody, { "X-Aion-Service-Token": AION_SERVICE_TOKEN }, 30000);
                if (!res.ok) {
        ...
        case "aion_channels": {
            ...
            try {
                ...
                if (!res.ok)
                    return ok({ error: `HTTP ${res.status}` });
        ...
        case "aion_channel_history": {
            ...
            try {
                const res = await aionGet(`/api/channels/${encodeURIComponent(channel)}/history?limit=${limit}`);
                if (!res.ok)
                    return ok({ error: `HTTP ${res.status}` });
                return ok(await res.json());
            }
        ...
        case "aion_channel_presence": {
            ...
            const { channel } = args;
            try {
                const res = await aionGet(`/api/channels/${encodeURIComponent(channel)}/presence`);
                if (!res.ok)
                    return ok({ error: `HTTP ${res.status}` });
                return ok(await res.json());
            }
        ...
        case "aion_admin_users": {
            ...
            try {
                ...
                if (!res.ok)
                    return ok({ error: `HTTP ${res.status} — need admin session` });
                return ok(await res.json());
            }
            catch (err) {
                return ok({ error: String(err) });
        ...
        case "aion_vast_stop": {
            ...
            try {
                const res = await aionPost(`/api/admin/vast/instances/${instance_id}/stop`, {}, {}, 15000);
                if (!res.ok)
                    return ok({ error: `HTTP ${res.status}` });
                return ok(await res.json());
            }
...
//# sourceMappingURL=aion-api-tools.js.map