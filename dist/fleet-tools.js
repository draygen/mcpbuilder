...
const SSH = {
    draydev: process.env.FLEET_DRAYDEV_SSH ??
        (DRAYDEV_PASS
...
const AGENT_CMD = {
    ...
    codex: 'F=$(mktemp); ERR=$(codex exec --skip-git-repo-check $M -o "$F" "$P" 2>&1 >/dev/null); ' +
...
// ---------------------------------------------------------------------------
async function runAgent(machine, agent, prompt, opts = {}) {
    ...
    const script = `export FLEET_DEPTH=${depth + 1}; ${cd}` +
        `M='${modelFrag}'; ` +
        `P="$(printf %s '${promptB64}' | base64 -d)"; ` +
        AGENT_CMD[agent];
    const scriptB64 = Buffer.from(script, "utf8").toString("base64");
    const decode = `echo ${scriptB64} | base64 -d | bash -l`;
    const full = machine === "wsl" ? decode : `${SSH[machine]} "${decode}"`;
    ...
    try {
        ...
        const e = err;
        if (e.killed)
            return `[fleet timeout after ${timeoutMs}ms on ${machine}/${agent}]`;
...
// Tool definitions
// ---------------------------------------------------------------------------
const MACHINES = ["wsl", "draydev", "ec2"];
const AGENTS = ["claude", "codex", "agy"];
export async function runFleetStatus(filter = {}) {
    ...
    return Promise.all(combos.map(async ({ m, a }) => {
...
export const fleetTools = [
    {
        name: "fleet_run",
        description: "Delegate a single prompt to another agent CLI (claude, codex, or agy) running headless " +
    ...
    {
        name: "fleet_review",
        description: "Fan out the same prompt (e.g. a diff to review, a design question) to multiple agents in " +
    ...
    {
        name: "fleet_status",
        description: "Health-check the fleet: confirm each agent CLI (claude/codex/agy) runs headless & " +
...
export async function handleFleetTool(name, args) {
    try {
        if (name === "fleet_run") {
            ...
            const machine = (String(args.machine ?? "draydev") || "draydev");
            if (!AGENTS.includes(agent))
                return fail(`agent must be one of: ${AGENTS.join(", ")}`);
            if (!MACHINES.includes(machine))
                return fail(`machine must be one of: ${MACHINES.join(", ")}`);
            if (!prompt)
        ...
        if (name === "fleet_review") {
            ...
            if (!MACHINES.includes(machine))
                return fail(`machine must be one of: ${MACHINES.join(", ")}`);
            const requested = Array.isArray(args.agents) ? args.agents.map(String) : ["codex", "agy"];
            const agents = requested.filter((a) => AGENTS.includes(a));
            if (agents.length === 0)
                return fail(`agents must be a subset of: ${AGENTS.join(", ")}`);
            const timeoutMs = args.timeout_ms ? Number(args.timeout_ms) : undefined;
            const results = await Promise.all(agents.map(async (a) => `===== ${a}@${machine} =====\n${await runAgent(machine, a, prompt, { timeoutMs })}`));
        ...
        if (name === "fleet_status") {
            ...
            const lines = checks.map((c) => `${c.ok ? "✓" : "✗"} ${c.agent}@${c.machine}: ${c.detail.slice(0, 80)}`);
